...
const fetchFileNames = async (user_id) => {
    try{
      ...
      const response = await fetch(`http://localhost:4000/getFileNames/${user_id}`);
  ...
  const fetchCode = async (fileName,user_id) => {
    try{
        const response = await fetch(`http://localhost:4000/getCode/${user_id}/${fileName}`);
        if(response.ok) { 
          const data = await response.json();
  ...
  const fetchCodeInRange = async (fileNames, start, end, user_id) => {
    try {
      ...
      const codePromises = fileNamesInRange.map((fileName) => fetchCode(fileName,user_id));
