...
const page = () => {
  ...
  useEffect(() => {
    ...
    const response = await fetch(`http://localhost:4000/fetchChallenge/${receivedChallengeId}`);
    if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
    }
    const data = await response.json();
    console.log(data);
    setNewChallengeName(data.challenge.challenge_name);
    setDescLong(data.challenge.description?.long);
    setDescShort(data.challenge.description?.short);
    setDifficulty(data.challenge.difficulty);
    setBoilerplate(data.challenge.boilerplate);
    setSolution(data.challenge.solution);
    setTestValues(data.challenge.test_values);
    }
    challengeFetch();
  }, [receivedChallengeId]);
  ...
  const EditChallenge = async (challengeName) => {
    try {
      if (!challengeName) {
      ...
      const newChallenge = {
        challengeId: receivedChallengeId,
      ...
      const response = await fetch(`http://localhost:4000/editChallenge/${receivedChallengeId}/`, {
        method: "PUT",
    ...
    //   console.log(data, response);
      if (response.ok) {
        notify("Challenge edited successfully ✅");
        ...
        <div className="container px-5 py-5 mx-auto flex sm:flex-nowrap flex-wrap">
          <div className="lg:w-1/3 md:w-1/2 bg-white flex flex-col md:ml-auto sm:mr-10 w-full p-10">
            ...
            <button
              ...
              onClick={() => {
                EditChallenge(newChallengeName);
              }}
            >
              Save Challenge
            </button>
